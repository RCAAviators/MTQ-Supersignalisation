const app = (function() {
	...
	const redraw = function() {
		...
		for (const panel of post.panels) {
			...
			if (panel.sign.guideArrow == "Left"){
				LeftArrowElmt = document.createElement("img");
				LeftArrowElmt.className = "arrow left";
				LeftArrowElmt.src = "img/arrows/MainArrows/B-14.svg";
				signElmt.appendChild(LeftArrowElmt);
			}

			if (panel.sign.guideArrow == "Ahead") {
				LeftArrowElmt = document.createElement("img");
				LeftArrowElmt.className = "arrow ahead";
				LeftArrowElmt.src = "img/arrows/MainArrows/B-15.svg";
				signElmt.appendChild(LeftArrowElmt);
			}
			...
			if (panel.sign.guideArrow == "Right") {
				LeftArrowElmt = document.createElement("img");
				LeftArrowElmt.className = "arrow right";
				LeftArrowElmt.src = "img/arrows/MainArrows/B-13.svg";
				signElmt.appendChild(LeftArrowElmt);
			}

			if (panel.sign.guideArrow == "SideRight") {
				LeftArrowElmt = document.createElement("img");
				LeftArrowElmt.className = "arrow sideright";
				LeftArrowElmt.src = "img/arrows/MainArrows/B-16.svg";
				signElmt.appendChild(LeftArrowElmt);
			}
			...
			for (const shield of panel.sign.shields) {
				...
				if (shield.type === "I-") {
					...
					if (shield.routeNumber.length <= 2) {
						shieldImgElmt.data = imgDir + "DO.svg";
					} else if (shield.routeNumber.length >= 3) {
						shieldImgElmt.data = imgDir + "DO.svg";
					}
					...
					if (shield.routeNumber.length <= 2) {
						...
						exitTabElmt.appendChild(bannerShieldContainerElmt)
					} else if (shield.routeNumber.length >= 3) {
						shieldImgElmt.data = imgDir + "Arp.svg";
					}
			...
			// Guide arrows
					...
					const createArrowElmt = function (key, dir, name, extra) {
						...
						if ((ExitKeys.includes(key.split("-")[0])) || (MainKeys.includes(key.split("-")[0]))) {
							...
							downArrowElmt.src = "img/arrows/" + directory + "/" + key + ".svg";
					...
					if ((panel.sign.arrowPosition == "Left") && (panel.sign.guideArrow != "Exit Only") && (panel.sign.guideArrow != "Side Left") && (panel.sign.guideArrow != "Side Right") && (panel.sign.guideArrow != "Half Exit Only")) {
						...
						arrowContElmt.style.paddingRight = "1rem";none
						if (panel.sign.actionMessage != "") {
							guideArrowsElmt.style.paddingBottom = "0.6rem";
						} else {
							guideArrowsElmt.style.paddingBottom = "1rem";
					...
					if ("Left" == panel.sign.guideArrow) {
						LeftArrowElmt.style.display = "block";
					} else if ("Right" == panel.sign.guideArrow) {
						LeftArrowElmt.style.display = "block";
					} else if ("Ahead" == panel.sign.guideArrow) {
						LeftArrowElmt.style.display = "block";
					} else if ("SideRight" == panel.sign.guideArrow) {
						LeftArrowElmt.style.display = "block";
						...
						if (panel.sign.guideArrow == "Left/Up Arrow:B-3" && panel.sign.otherSymbols != "") {
							arrowContElmt.appendChild(otherSymbolsElmt);
							arrowContElmt.className += ` rightText`;
						}
						if (panel.sign.guideArrow == "Right/Down Arrow:B-4" && panel.sign.otherSymbols != "") {
							arrowContElmt.appendChild(otherSymbolsElmt);
							arrowContElmt.className += ` rightText`;
						}
			...
			switch(panel.sign.otherSymbol) {
				...
				case "Quebec-2":
				case "Quebec-2-Left":
					if (panel.sign.otherSymbol == "Quebec-Style Exit Marker") {
					...
					if (panel.sign.otherSymbol == "Quebec-2") {
						const marker2Elmt = document.createElement("object");
						marker2Elmt.className = "marker2Img";
						marker2Elmt.type = "image/svg+xml";
						marker2Elmt.data = "img/other-symbols/QC-Exit2.svg";
						oSNumElmt.style.top = "0.55rem";
						otherSymbolsElmt.appendChild(marker2Elmt);
					}
